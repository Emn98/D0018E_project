<?php
  ...
  if(isset($_POST["product_id"])){
    ...
    if($quantity_in_cart != ""){
      ...
      $query = $con->prepare("SELECT quantity FROM PRODUCT_INVENTORY WHERE product_id=? and color=?");
      $query->bind_param("is", $product_id, $product_color);
      $query->execute();
      $query->bind_result($quantity_in_stock);
      $query->fetch();
      $query->close();
